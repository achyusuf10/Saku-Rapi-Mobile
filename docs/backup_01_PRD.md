...
- Scope:
  ...
  - specific wallet
- Auto-renew opsional (pg_cron daily job `auto_renew_budgets()` — **aktif**).
- Threshold alert:
  - 50% — `notification_sent_50` (info, opsional via setting `budget_alert_50_enabled`)
  - 80% — `notification_sent_80`
  - 100% — `notification_sent_100`
- Alert dicek saat halaman budget di-load via `BudgetAlertChecker`.
- **Carry-forward (opsional):** Jika `carry_forward = true` dan budget recurring, sisa positif dari periode lama ditambahkan ke nominal budget baru. Sisa negatif (over budget) **tidak** dikurangi; budget baru mulai dengan nominal asli.
...
- **Custom** — date range picker. Validasi: `start_date >= today` saat create.

### Auto-renew logic (pg_cron)
Untuk tipe periode standar:
- **Weekly**: new_start = old_end + 1, new_end = new_start + 6 days.
- **Monthly**: new_start = 1st of next month, new_end = last day of next month.
- **Quarterly**: new_start = 1st of next quarter, new_end = last day of next quarter.
- **Yearly**: new_start = Jan 1 next year, new_end = Dec 31 next year.

Untuk tipe **Custom** recurring:
- Tentukan apakah `end_date` = hari terakhir bulannya.
- `new_start` = tanggal sama di bulan berikutnya (clamp jika bulan tak punya tanggal itu).
- Jika end = akhir bulan → `new_end` = hari terakhir bulan dari new end's month.
- Jika end ≠ akhir bulan → `new_end` = `new_start` + durasi asli (jumlah hari).
- Contoh 1: Jan 10–31 (akhir bulan) → Feb 10–28.
- Contoh 2: Jan 10–29 (20 hari) → Feb 10–Mar 1 (20 hari).

Carry-forward saat renew:
- Jika `carry_forward = true` dan `remaining > 0`: `new_amount = amount + remaining`.
- Jika `carry_forward = true` dan `remaining <= 0`: `new_amount = amount` (tidak kurangi).
...
- **Period type tabs:** Dynamic TabBar menampilkan hanya tipe periode yang ada.
  - Untuk budget bertipe `custom`, setiap budget custom mendapat tab terpisah dengan label tanggal (`dd/MM – dd/MM`), bukan satu tab "Custom" gabungan.
- **BudgetSummaryCard:**
  ...
  - Label "Spendable" + jumlah tersisa.
    - Jika positif: hijau, tampilkan nilai sisa.
    - Jika negatif (over budget): merah, tampilkan nilai negatif (misal "−Rp 200.000").
...
- **Section "Budget Mendatang":** Menampilkan budget dengan `start_date > today`. User dapat tap untuk detail, edit, atau hapus.
...
- **Progress section:**
  - Spent vs Remaining (kolom berwarna). Remaining menampilkan negatif jika over budget (warna merah).
...
- **Transaction list:** semua transaksi expense dalam periode + kategori budget, **termasuk transaksi sub-kategori** (child categories). Query mengambil child category IDs dari database secara terpisah agar list transaksi konsisten dengan `used_amount`.
- **AppBar actions:** Edit (buka form sheet), Delete (konfirmasi dialog).
- **Setelah edit sukses:** Halaman detail di-refresh dengan data budget terbaru, **bukan** langsung pop ke list. User tetap di halaman detail dan melihat perubahan yang baru dilakukan.
...
- **Pagination:** Menggunakan `VisibilityDetector` pada item terakhir untuk infinite scroll (page size: 20). Global widget `LoadMoreFooter` untuk menampilkan loading, end-of-data, atau error+retry.

### BudgetFormSheet (Full-screen)
**Fields:**
| Field | Tipe | Wajib | Default |
|---|---|---|---|
| Category | Picker (expense only) | Ya | — |
| Amount | Currency field (> 0) | Ya | 0 |
| Period | Presets + Custom | Tidak | Inherit dari tab aktif, fallback This Month |
| Wallet scope | Global / per-wallet | Tidak | Global |
| Recurring | Toggle checkbox | Tidak | false |
| Carry-forward | Toggle checkbox (hanya muncul jika Recurring = true) | Tidak | false |
...
- Custom (date range picker) — **start_date >= today** saat create mode.

**Period detection saat edit:**
- Deteksi semua tipe periode (weekly, monthly, quarterly, yearly) berdasarkan tanggal start/end budget, bukan hanya "bulan ini".
- Jika cocok dengan salah satu preset → tampilkan label preset.
- Jika tidak cocok → tampilkan "Custom" dengan rentang tanggal.

**Duplicate detection (seragam untuk create & edit):**
- Saat submit, controller memanggil `findDuplicateBudgetId(categoryId, walletId, startDate, endDate, excludeBudgetId?)`.
- Jika duplikat ditemukan → dialog konfirmasi:
  - **"Ganti"**: hapus old + buat new secara atomik via RPC `replace_budget`.
  - **"Batal"**: batalkan operasi.
- Jika tidak ada duplikat → create/update langsung.
- **Baik create maupun edit** mendapat dialog yang sama jika terdeteksi duplikat.

**Replace budget (atomik):**
- Menggunakan RPC `replace_budget(p_old_budget_id, p_user_id, p_category_id, ...)` yang menjalankan DELETE + INSERT dalam satu transaksi database. Jika salah satu gagal, keduanya di-rollback.

**Konfirmasi tutup form (dirty check):**
- Jika user sudah mengisi field (state berubah dari initial) lalu menekan tombol close (×), tampilkan dialog konfirmasi: "Perubahan belum disimpan. Yakin keluar?".
- Jika form belum dimodifikasi, tutup langsung tanpa konfirmasi.
...
4. Filter budget dengan `isHalfUsed` (≥ 50%) **DAN** belum near limit → kirim alert info jika `notification_sent_50 == false` **DAN** setting `budget_alert_50_enabled = true`.
5. Prioritas: 100% > 80% > 50%. Alert tertinggi yang aktif saja yang dikirim.
6. Setelah kirim, update flag di database via `markBudgetNotificationSent()`.

**Prasyarat:** `budget_alert_enabled = true` pada `notification_settings`. Untuk 50%: tambahan `budget_alert_50_enabled = true`.
...
**Konten notifikasi:**
- 50%: "Anggaran {category} sudah terpakai 50%"
- 80%: "Anggaran {category} sudah mencapai 80%"
- 100%: "Anggaran {category} sudah melampaui batas!"

**Reset:** Flag `notification_sent_50`, `notification_sent_80`, dan `notification_sent_100` di-reset oleh `auto_renew_budgets` pg_cron job saat budget di-clone ke periode baru.

### Acceptance criteria
- Expense pada child category dapat mengurangi budget parent.
- Scope wallet bekerja benar.
- Threshold 50%, 80%, dan 100% hanya terkirim sekali per periode.
- Period tabs menampilkan tipe yang memiliki budget aktif; budget custom mendapat tab terpisah dengan label tanggal.
- Duplicate detection seragam di create dan edit — dialog yang sama ditampilkan.
- Replace budget berjalan atomik via RPC (`replace_budget`).
- Budget usage (used_amount) diupdate otomatis via trigger dari transaction_items.
- **Transaksi sub-kategori tampil** di halaman detail budget parent (konsisten dengan used_amount).
- BudgetAlertChecker triggered saat halaman budget load, bukan via background worker.
- Alert 100% diprioritaskan; jika sudah over 100%, alert 80% dan 50% tidak dikirim.
- **pg_cron auto-renew aktif** — budget recurring otomatis di-clone ke periode berikutnya.
- **Carry-forward** menambah sisa positif ke nominal budget baru.
- **Budget masa depan** (upcoming) terlihat dan bisa di-edit/hapus.
- **Completed budgets** menggunakan pagination infinite scroll.
- **Spendable** menampilkan nilai negatif (bukan 0) saat over budget.
- **Detail page** di-refresh setelah edit, user tetap di halaman detail.
- **Form** mendeteksi semua periode (bukan hanya bulan ini) saat edit.
- **Form** menampilkan konfirmasi saat ditutup jika ada perubahan belum disimpan.
- **Custom range** hanya bisa mulai dari hari ini saat create.
- **Custom recurring** mempertahankan akhir bulan atau durasi sesuai konfigurasi asal.
