...
/// Horizontal scrollable tab bar untuk navigasi sub-period di Report.
///
/// Menampilkan tab per sub-period (misal setiap bulan untuk mode "Monthly")
/// hingga beberapa periode ke belakang. Tab terakhir selalu "saat ini" dan
/// otomatis di-scroll ke sana saat pertama kali tampil atau saat period berganti.
class ReportSubPeriodTabs extends ConsumerStatefulWidget {
  const ReportSubPeriodTabs({super.key});

  @override
  ConsumerState<ReportSubPeriodTabs> createState() =>
      _ReportSubPeriodTabsState();
}

class _ReportSubPeriodTabsState extends ConsumerState<ReportSubPeriodTabs> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final selectedIndex =
          state.subPeriodIndex ?? (state.subPeriodTabs.length - 1);
  ...
  Widget build(BuildContext context) {
    ...
    final selectedIndex = state.subPeriodIndex ?? (tabs.length - 1);

    // Auto-scroll saat sub-period atau period berubah
    if (_lastIndex != selectedIndex || _lastPeriod != state.period) {
