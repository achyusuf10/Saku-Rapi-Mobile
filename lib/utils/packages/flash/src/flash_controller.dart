...
typedef FlashBuilder<T> =
    Widget Function(BuildContext context, FlashController<T> controller);
...
class DefaultFlashController<T> implements FlashController<T> {
  ...
  List<OverlayEntry> _createOverlayEntries() {
    return <OverlayEntry>[
      if (hasBarrier)
        OverlayEntry(builder: (context) => _buildBarrier(context)),
      OverlayEntry(
        builder: (context) =>
            _FlashScope(controller: this, child: builder(context, this)),
        maintainState: true,
      ),
    ];
  }
  ...
  bool get hasBarrier =>
      barrierDismissible || barrierColor != null || barrierBlur != null;
...
class __FlashScopeState extends State<_FlashScope> {
  final focusScopeNode = FocusScopeNode(
    debugLabel: '$__FlashScopeState Focus Scope',
  );
  ...
  void initState() {
    ...
    if (widget.controller.hasBarrier &&
      ...
      widget.controller.route?.navigator!.focusNode.enclosingScope
